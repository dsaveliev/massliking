    ...
    <wrapper>
      <div class="card shadow-3 channel-form-card">
        <div class="card-title bg-wite text-pink">
          <big class="channel-form-title">{{ $t("channel.titles.edit") }}</big>
        </div>
        <div class="card-content card-force-top-padding column">
          ...
          <div class="channel-input">
            <button class="deep-purple big full-width" @click="submit">{{ $t("channel.buttons.edit") }}</button>
          </div>
...
export default {
  ...
  mounted: function () {
    this.form.action = this.channel(this.instagram_id, this.id).action
    this.form.target = this.channel(this.instagram_id, this.id).target
    this.form.value = this.channel(this.instagram_id, this.id).value
    this.form.comments = this.channel(this.instagram_id, this.id).comments
  },
  methods: {
    submit () {
      this.$v.form.$touch()
