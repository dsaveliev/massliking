<template>
  <div class="insta-form-container">
    <div class="row no-wrap gt-sm">
      ...
      <router-link :to="{ name: 'instagram-show', params: { instagram_id: instagram.id }}">
        <button class="deep-purple insta-button-nav">{{ $t("instagram.buttons.show") }}</button>
      </router-link>
    </div>
    ...
    <div class="row wrap sm">
      ...
      <router-link :to="{ name: 'instagram-show', params: { instagram_id: instagram.id }}">
        <button class="deep-purple insta-button-nav full-width">{{ $t("instagram.buttons.show") }}</button>
      </router-link>
    </div>
    ...
    <wrapper>
      <div class="card shadow-3 insta-form-card">
        <div class="card-title bg-wite text-pink">
          <big class="insta-form-title">{{ $t("instagram.titles.edit") }}</big>
        </div>
        <div class="card-content card-force-top-padding column">
          ...
          <div class="insta-input">
            <button class="deep-purple big full-width" @click="submit">{{ $t("instagram.buttons.edit") }}</button>
          </div>
...
export default {
  ...
  mounted: function () {
    this.form.username = this.instagram.username
    this.form.password = this.instagram.password
    this.form.trusted = this.instagram.trusted
    this.form.hours.min = this.instagram.hours.min
    this.form.hours.max = this.instagram.hours.max
    this.form.speed.like = this.instagram.speed.like
    this.form.speed.comment = this.instagram.speed.comment
    this.form.speed.follow = this.instagram.speed.follow
    this.form.speed.unfollow = this.instagram.speed.unfollow
  },
  validations: {
    form: {
      username: { required },
  ...
  computed: {
    instagram () {
      return this.$store.getters.findInstagram(this.$route.params.instagram_id)
    }
  },
  watch: {
  ...
  methods: {
    submit () {
      ...
      InstagramAPI.editInstagram(this, this.instagram, this.form, '/instagram/' + this.instagram.id)
    }
  }
}
</script>
